    ...
    public static class MvcFormExtensions
    {
            ...
            this ComponentBuilder<MvcBootstrapConfig<TModel>, Select> builder, IEnumerable<SelectListItem> selectList)
        {
            if (selectList != null)
            {
                ...
                {
                    SelectListItem item1 = item; // Avoid foreach variable access in closure
        ...
        {
            ControlLabel controlLabel = GetControlLabel(builder.GetHelper(), expression).For(TagBuilder.CreateSanitizedId(builder.GetComponent().GetAttribute("name"), "_")).GetComponent();
            if (labelAction != null)
            {
                labelAction(controlLabel);
            }
        ...
        {
            TTag formControl = builder.GetComponent();
